...
from module import health_assessment
...
if __name__ == "__main__":
    ...
    aoi_dict = {
        'AOI1': {
            ...
            'fnf_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'lazio_ft.tif'),
            'ft_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'lazio_ft.tif'),
            'ft_cod_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'Legend_S4_07_10_qd_RB.xlsx')
        },
        'AOI2': {
            'shapefile': 'AOI_SVC_S4-07C-10_AOI2.shp',
            'mgrs_tile': '32TQS',
            'fnf_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'trentino_ft.tif'),
            'ft_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'trentino_ft.tif'),
            'ft_cod_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'Legend_S4_07_10_qd_RB.xlsx')
        },
        'AOI3': {
            'shapefile': 'AOI_SVC_S4-07B_AOI3.shp',
            'mgrs_tile': '33TUG',
            'fnf_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'abruzzo_ft.tif'),
            'ft_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'abruzzo_ft.tif'),
            'ft_cod_relative_path': os.path.join('PM-2.05_Other_data_processing', 'carteforestali', 'Legend_S4_07_10_qd_RB.xlsx')
        }
    }
    ...
    aoi_geometry = gpd.read_file(os.path.join(shp_path, aoi_dict[aoi]['shapefile'])).dissolve().geometry
    aoi_geometry_4326 = gpd.read_file(os.path.join(shp_path, aoi_dict[aoi]['shapefile'])).to_crs(4326).dissolve().geometry
    ...
    if assessment_kind == 'yearly':
        if chain == 'SVC10C':
            start_date_pre = pd.to_datetime(start_date) + pd.DateOffset(years=-offset[-1])
            end_date_pre = pd.to_datetime(end_date) + pd.DateOffset(years=-offset[-1])
            for years_offset in offset:
                start_date_post = pd.to_datetime(start_date_pre) + pd.DateOffset(years=years_offset)
                end_date_post = pd.to_datetime(end_date_pre) + pd.DateOffset(years=years_offset)
                time_ranges.append((start_date_pre, end_date_pre, start_date_post, end_date_post))
        elif chain == 'SVC07C':
            start_date_post = pd.to_datetime(start_date)
            end_date_post = pd.to_datetime(end_date)
            for years_offset in offset:
                start_date_pre = pd.to_datetime(start_date_post) + pd.DateOffset(years=-years_offset)
                end_date_pre = pd.to_datetime(end_date_post) + pd.DateOffset(years=-years_offset)
                time_ranges.append((start_date_pre, end_date_pre, start_date_post, end_date_post))
        elif chain == 'SVC07B':
            for years_offset in offset:
                start_date_pre = pd.to_datetime(start_date) + pd.DateOffset(years=-years_offset)
                end_date_pre = pd.to_datetime(end_date) + pd.DateOffset(years=-years_offset)
                start_date_post = pd.to_datetime(start_date) + pd.DateOffset(years=years_offset)
                end_date_post = pd.to_datetime(end_date) + pd.DateOffset(years=years_offset)
                start_date_mid = pd.to_datetime(start_date)
                end_date_mid = pd.to_datetime(end_date)
                time_ranges.append(
                    (start_date_pre, end_date_pre, start_date_mid, end_date_mid, start_date_post, end_date_post))
        ...
        for days_offset in offset:
            ...
            end_date_pre = date + pd.DateOffset(days=14, hours=23, minutes=59, seconds=59)
            start_date_post = date + pd.DateOffset(days=15)
            end_date_post = date + pd.DateOffset(days=29, hours=23, minutes=59, seconds=59)
            time_ranges.append((start_date_pre, end_date_pre, start_date_post, end_date_post))

    client = Client(dashboard_address=dask_dashboard_address, n_workers=dask_n_workers, memory_limit=dask_memory_limit)
    ...
    for i, time_range in enumerate(time_ranges):
        ...
        if chain == "SVC07B":
            start_date_pre, end_date_pre, start_date_mid, end_date_mid, start_date_post, end_date_post = time_range
        else:
            start_date_mid = None
            end_date_mid = None
