...
class TestForestHealth:
    ...
    def query_to_ds(self):
        ...
        time_range = (
            start_date_pre.strftime("%Y-%m-%d")
            + "/"
            + end_date_pre.strftime("%Y-%m-%d")
        )
    ...
    def test_data_vars(self, query_to_ds):
        assert list(query_to_ds.data_vars) == [
            "B02",
            "B03",
            "B04",
            "B05",
    ...
    def test_isel_func(self, query_to_ds):
        from chains.module import isel_func
    ...
    def test_coords_to_attrs(self, query_to_ds):
        from chains.module import coords_to_attrs

        cleaned_ds = coords_to_attrs(query_to_ds)
        assert list(cleaned_ds.coords) == [
            "time",
            "x",
            "y",
        ...
        expected = {
            ...
            "s2:datastrip_id": "['S2A_OPER_MSI_L2A_DS_ESRI_20200910T215436_S20200525T100503_N02.12'\n "
            ...
            "s2:product_uri": "['S2A_MSIL2A_20200525T100031_N0212_R122_T33TTG_20200910T215435.SAFE'\n "
            ...
            "s2:mgrs_tile": "33TTG",
            "s2:granule_id": "['S2A_OPER_MSI_L2A_TL_ESRI_20200910T215436_A025716_T33TTG_N02.12'\n "
    ...
    def test_s2l2a_masked(self, query_to_ds):
        ...
        assert expected_ds.equals(
            actual_ds
        ), "function s2l2a_masked returns different dataset"

    def test_medoid_mosaic(self, query_to_ds):
        from chains.module import coords_to_attrs, medoid_mosaic, s2l2a_masked
